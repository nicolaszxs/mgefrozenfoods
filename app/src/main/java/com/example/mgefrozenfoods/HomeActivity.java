...
public class HomeActivity extends AppCompatActivity {
    ...
    private void processOrder() {
        ...
        if (address.isEmpty() || contact.isEmpty()) {
            Toast.makeText(this, "Please enter address and contact", Toast.LENGTH_SHORT).show();
            return;
        }

        StringBuilder itemsSummary = new StringBuilder();
        for (CartItem item : cartList) {
            itemsSummary.append(item.getQuantity()).append("x ").append(item.getProduct().getName()).append("; ");
        }

        SharedPreferences prefs = getSharedPreferences("MGE_ADMIN_DATA", MODE_PRIVATE);
        String existing = prefs.getString("ALL_ORDERS", "");
        String orderEntry = "Order#" + (System.currentTimeMillis() % 100000) + "||"
                + tvProfileUsername.getText() + "||"
                + address + "||"
                + itemsSummary.toString() + "||"
                + currentGrandTotal + "||Pending";

        prefs.edit().putString("ALL_ORDERS", orderEntry + "##" + existing).apply();
        ...
        if (!data.isEmpty()) {
            for (String o : data.split("##")) {
                ...
                if (p.length >= 6 && p[1].equals(user)) {
                    double price = 0.0;
                    try {
                        price = Double.parseDouble(p[4]);
                    } catch (Exception e) {}
                    orderList.add(new Order(p[0], "Status: " + p[5], p[5], p[3], price));
                }
            }
        }
